...
fn get_search_data() -> Option<(String, String, Vec<String>)>{
    let search_path = match get_input("Enter the path to the dir to search for file in: "){
        Ok(path) => path,
        Err(_) => return None
    };

    let search_name = match get_input("Enter the filename to search (without extension): "){
        Ok(path) => path,
        Err(_) => return None
    };

    let search_extensions = match get_input("Enter the file extensions separated by whitespace: "){
